...
import com.example.konvergence.model.ChoiceQuestionCount;
import com.example.konvergence.model.Submission;
...
public interface SubmissionRepository extends JpaRepository<Submission, Long> {
    @Query("SELECT NEW com.example.konvergence.model.ChoiceQuestionCount(v.choice.id, count(v.id)) FROM Submission v WHERE v.question.id in :questionIds GROUP BY v.choice.id")
    List<ChoiceQuestionCount> countByPollIdInGroupByChoiceId(@Param("questionIds") List<Long> questionIds);

    @Query("SELECT NEW com.example.konvergence.model.ChoiceQuestionCount(v.choice.id, count(v.id)) FROM Submission v WHERE v.question.id = :questionId GROUP BY v.choice.id")
